...
public function cashfree($order_id,$amount)
{
	$payload = [
    "order_id" => $order_id,
    "order_amount" => $amount,
...
public function cashfree_gateway(Request $request)
{
    ...
    $user_id     = $request->user_id;
    $generate_OrderId = 'ORD_' . $user_id . '_' . time();
    $cashfree = $this->cashfree($generate_OrderId, $totalamount);
    $cashfree_data = json_decode(trim($cashfree), true);

    
    if (empty($cashfree_data['payment_session_id'])) {
       
        $generate_OrderId = 'ORD_' . $user_id . '_' . time() . '_retry';
        $cashfree = $this->cashfree( $totalamount);
        $cashfree_data = json_decode(trim($cashfree), true);
    }

    $payment_session_id = $cashfree_data['payment_session_id'] ?? null;

    if (!$payment_session_id) {
        return response()->json([
            'status'  => false,
            'message' => $cashfree_data['message'] ?? 'Unable to create payment session',
            'data'    => $cashfree_data,
        ]);
    }
